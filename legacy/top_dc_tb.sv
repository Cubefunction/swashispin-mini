...
module top_dc_tb;
    ...
    for (genvar i = 0; i < NUM_CHANNEL; i++) begin : GEN_EMPTY_IDLE
        assign w_dc_empty_bus[i] = dut.u_uart_api_dc.GEN_DC[i].u_dc.w_empty;
        assign w_dc_idle_bus[i] = dut.u_uart_api_dc.GEN_DC[i].u_dc.core.r_state == dut.u_uart_api_dc.GEN_DC[i].u_dc.core.IDLE;
    end
